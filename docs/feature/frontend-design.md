...
2. **시트 뷰어**: 사이드바(시트 목록 + 시트 추가/삭제) + 테이블(번역 데이터). 소스 셀 포함 모든 셀 인라인 편집 가능. 행(key) 추가 + 체크박스 선택 일괄 삭제. 번역/업데이트/검수 실행 트리거.
...
**In scope (v0)**: 6개 화면 전체, 프로젝트 CRUD, 시트 추가/삭제, 행(key) 추가/일괄 삭제, 번역 실행/모니터링
...
- 시트 설정 화면 (Screen 3) — 소스 언어 선택, 번역 스타일, 용어집 오버라이드, 글자수 제한, 자유 지시사항
