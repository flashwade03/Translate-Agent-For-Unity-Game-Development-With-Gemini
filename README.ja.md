...
**Google ADK (Gemini)** ベースの3エージェントアーキテクチャで、翻訳・レビュー・オーケストレーションを実行します。**ローカルCSVファイル**（Unity Localization互換）と **Google Sheets**（Google Workspace CLI連携）の両方に対応しています。
...
**Google Sheets連携**
- プロジェクトをGoogle Sheetsスプレッドシートに直接接続
- エージェントで翻訳 → ダッシュボードで変更を確認 → Sheetsに適用
- 保留中の翻訳は適用するまでローカル（SQLite）に保存
- タブ、行、言語をシートヘッダーから自動検出
...
│   ├── agent.py               #   ルートオーケストレーター + エージェントファクトリー
│   ├── prompts.py             #   エージェントプロンプト
│   ├── sub_agents/            #   Translator & Reviewer
│   ├── tools/                 #   CSV読み書き、gws読み取り、保留保存、設定、用語集
│   └── skills/                #   ADKスキル定義（gws_sheets）
├── backend/                   # FastAPIサーバー
│   ├── routers/               #   APIルート（sheets, jobs, translations, gws...）
│   └── services/              #   ビジネスロジック（gws_service, pending_translations...）
...
| ストレージ | ローカルCSV + YAML、Google Sheets（gws CLI連携）、SQLite（ジョブ/保留） |
| CLI | Click |
