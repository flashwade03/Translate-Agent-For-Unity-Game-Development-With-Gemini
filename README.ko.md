...
**Google ADK (Gemini)** 기반의 3-에이전트 아키텍처로 번역, 리뷰, 오케스트레이션을 수행합니다. 모든 데이터는 **Unity Localization** 호환 로컬 CSV 파일로 저장됩니다.
